let recipes = [
    ...
    },
    {
        ...
        for (let i = 0; i < recipes.length; i++) {
            container.innerHTML += `<div class="card ${recipes[i].difficulty.toLowerCase()} ${recipes[i].cuisine.toLowerCase()}">
                    ...
                    <button style="color: #edf4f7;background-color: #0ec9f8;width:120px;height:5vh;border-radius:10px;">See Details</button>
                </div>`;

            if (!cuisineArray.includes(recipes[i].cuisine)) {
                cuisineArray.push(recipes[i].cuisine);
            }
            if (!diffArray.includes(recipes[i].difficulty)) {
                diffArray.push(recipes[i].difficulty);
            }
        }

        let cuisineSelect = document.getElementById("cuis");
        for (let i = 0; i < cuisineArray.length; i++) {
            cuisineSelect.innerHTML += `<option class="sel" value="${cuisineArray[i].toLowerCase()}">${cuisineArray[i]}</option>`;
        }

        let diffSelect = document.getElementById("diff");
        for (let i = 0; i < diffArray.length; i++) {
            diffSelect.innerHTML += `<option class="sel" value="${diffArray[i].toLowerCase()}">${diffArray[i]}</option>`;
        }
        ...
        function filterCards() {
            let selectedDifficulty = diffSelect.value.toLowerCase();
            let selectedCuisine = cuisineSelect.value.toLowerCase();
            let cards = document.getElementsByClassName("card");

            for (let i = 0; i < cards.length; i++) {
                let card = cards[i];
                let cardDifficulty = card.classList.contains(selectedDifficulty) || selectedDifficulty === "";
                let cardCuisine = card.classList.contains(selectedCuisine) || selectedCuisine === "";
